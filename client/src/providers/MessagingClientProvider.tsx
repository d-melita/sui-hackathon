...
const MessagingClientContext = createContext<SuiStackMessagingClient | null>(
  null,
);
...
export const useMessagingClient = (): SuiStackMessagingClient | null => {
  ...
  if (ctx === undefined) {
    throw new Error(
      "useMessagingClient must be used within a MessagingClientProvider",
    );
  }
  return ctx;
};
...
export const MessagingClientProvider = ({
  ...
  const messagingClient = useMemo(() => {
    ...
    try {
      ...
      return messagingClient.messaging;
    } catch (error) {
