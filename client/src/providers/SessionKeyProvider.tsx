...
const SessionKeyContext = createContext<SessionKeyContextProps | undefined>(
  undefined,
);

const PACKAGE_ID =
  "0x984960ebddd75c15c6d38355ac462621db0ffc7d6647214c802cd3b685e1af3d";
const TTL_MINUTES = 30; // Maximum allowed by SDK
...
export const SessionKeyProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Auto-refresh session key before expiration
  const setupAutoRefresh = (sessionKey: SessionKey) => {
    // Clear any existing timer
    if (refreshTimer) {
      clearTimeout(refreshTimer);
    }

    // Since TTL is 30 minutes, refresh after 25 minutes (5 minutes before expiry)
    ...
    const timer = setTimeout(async () => {
      console.log("🔄 Auto-refreshing session key...");
      try {
  ...
  const initializeManually = async () => {
    ...
    try {
      ...
      // Set up auto-refresh for the new session key
      setupAutoRefresh(newSessionKey);
    } catch (err) {
      console.error("Error initializing session key:", err);
      setError(
        err instanceof Error
          ? err
          : new Error("Failed to initialize session key"),
      );
      setSessionKey(null);
    } finally {
      ...
      if (cachedSessionData) {
        ...
        try {
          ...
          if (!restoredSessionKey.isExpired()) {
            ...
            // Set up auto-refresh for the restored session key
            setupAutoRefresh(restoredSessionKey);
            return;
          } else {
            console.log(
              "Cached session key is expired, manual initialization required",
            );
            clearSessionKey(currentAccount.address, PACKAGE_ID);
            setSessionKey(null);
          }
        ...
        console.log(
          "No cached session key found, manual initialization required",
        );
        setSessionKey(null);
      }
    };
  ...
  }, [currentAccount?.address, sessionKey, refreshTimer]);
