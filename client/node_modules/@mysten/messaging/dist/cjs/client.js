...
getGeneratedCaps_fn = async function(digest) {
  const creatorCapType = import_creator_cap.CreatorCap.name.replace(
    "@local-pkg/sui-stack-messaging",
    __privateGet(this, _packageConfig).packageId
  );
  const creatorMemberCapType = import_member_cap.MemberCap.name.replace(
    "@local-pkg/sui-stack-messaging",
    __privateGet(this, _packageConfig).packageId
  );
  const additionalMemberCapType = import_member_cap.MemberCap.name.replace(
    "@local-pkg/sui-stack-messaging",
    __privateGet(this, _packageConfig).packageId
  );
  const {
  ...
  const creatorCapParsed = import_creator_cap.CreatorCap.parse(await suiCreatorCapObject.content);
  ...
  const creatorMemberCapParsed = import_member_cap.MemberCap.parse(await suiCreatorMemberCapObject.content);
  ...
  const additionalMemberCapsParsed = await Promise.all(
    suiAdditionalMemberCapsObjects.map(async (object) => {
      ...
      return import_member_cap.MemberCap.parse(await object.content);
    })
  );
